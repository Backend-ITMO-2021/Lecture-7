...
object NonEmpty {
  ...
  lazy val applicative: Applicative[NonEmpty] = new Applicative[NonEmpty] {
    def point[A](a: A): NonEmpty[A] = NonEmpty(a, List())
    
    def ap[A, B](fa: NonEmpty[A])(f: NonEmpty[A => B]): NonEmpty[B] = f match {
      case NonEmpty(head, Nil) => NonEmpty(
        head(fa.head),
        for {fas <- fa.list} yield head(fas)
      )
      case NonEmpty(head, x :: xs) => {
        val y = ap(fa)(NonEmpty(head, Nil))
        val ys = ap(fa)(NonEmpty(x, xs))
        NonEmpty(y.head, y.list ++ (ys.head :: ys.list))
      }
    }

    def map[A, B](fa: NonEmpty[A])(f: A => B): NonEmpty[B] = functor.map(fa)(f)
  }
  ...
  lazy val monad: Monad[NonEmpty] = new Monad[NonEmpty] {
    def flatMap[A, B](fa: NonEmpty[A])(f: A => NonEmpty[B]): NonEmpty[B] = fa match {
      ...
      case NonEmpty(head, x :: xs) => {
        val y = f(head)
        val ys = flatMap(NonEmpty(x, xs))(f)
        NonEmpty(y.head, y.list ++ (ys.head :: ys.list))
      }
    }

    def point[A](a: A): NonEmpty[A] = applicative.point(a)
  }
}
